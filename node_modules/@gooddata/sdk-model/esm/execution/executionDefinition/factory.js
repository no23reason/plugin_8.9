...
import { isDimension, MeasureGroupIdentifier, newDimension, newTwoDimensional, } from "../base/dimension";
import { bucketAttributes, bucketMeasures } from "../buckets";
import { bucketsAttributes, bucketsIsEmpty, bucketsMeasures } from "../buckets/bucketArray";
import { insightBuckets, insightFilters, insightSorts } from "../../insight";
import { isMeasure } from "../measure";
import { defSetDimensions, defSetPostProcessing, defSetSorts, defWithFilters, defSetExecConfig, } from "./index";
...
export function newDefForItems(workspace, items, filters) {
    ...
    var def = __assign(__assign({}, emptyDef(workspace)), { attributes: items.filter(isAttribute), measures: items.filter(isMeasure) });
    defValidate(def);
...
export function newDefForBuckets(workspace, buckets, filters) {
    ...
    var def = __assign(__assign({}, emptyDef(workspace)), { buckets: buckets, attributes: bucketsAttributes(buckets), measures: bucketsMeasures(buckets) });
    defValidate(def);
    return defWithFilters(def, filters);
}
/**
...
export function newDefForInsight(workspace, insight, filters) {
    ...
    var def = newDefForBuckets(workspace, insightBuckets(insight));
    defValidate(def);
    var extraFilters = filters ? filters : [];
    var filteredDef = defWithFilters(def, __spreadArrays(insightFilters(insight), extraFilters));
...
export function defWithDateFormat(definition, dateFormat) {
    var currentPostProcessing = definition.postProcessing || {};
    var postProcessing = __assign(__assign({}, currentPostProcessing), { dateFormat: dateFormat });
 ...
 * @public
 */
export function defWithDimensions(definition) {
    var dims = [];
...
function toDimensions(dimsOrGen, def) {
    ...
    var maybeGenerator = dimsOrGen[0];
...
function defaultDimensionsWithBuckets(buckets) {
    var firstBucket = buckets[0], otherBuckets = buckets.slice(1);
    if (bucketsIsEmpty(otherBuckets)) {
        if (bucketMeasures(firstBucket).length) {
            return newTwoDimensional([MeasureGroupIdentifier], bucketAttributes(firstBucket).map(attributeLocalId));
        }
        return [newDimension(bucketAttributes(firstBucket).map(attributeLocalId))];
    }
    var firstDim = bucketAttributes(firstBucket).map(attributeLocalId);
    var secondDim = bucketsAttributes(otherBuckets).map(attributeLocalId);
    if (bucketMeasures(firstBucket).length) {
...
export function defaultDimensionsGenerator(definition) {
    ...
    var buckets = definition.buckets;

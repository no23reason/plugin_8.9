...
import { ALL_SCREENS, DASHBOARD_LAYOUT_CONTAINER_WIDTHS, DASHBOARD_LAYOUT_GRID_COLUMNS_COUNT, DASHBOARD_LAYOUT_MAX_HEIGHT_AS_RATIO_XS, GRID_ROW_HEIGHT_IN_PX, } from "../constants";
...
import { INSIGHT_WIDGET_SIZE_INFO_DEFAULT_LEGACY, KPI_WIDGET_SIZE_INFO_DEFAULT, KPI_WIDGET_SIZE_INFO_DEFAULT_LEGACY, INSIGHT_WIDGET_SIZE_INFO_DEFAULT, } from "../../../components/pluggableVisualizations/constants";
export function unifyDashboardLayoutItemHeights(itemsOrLayout) {
    ...
    return ALL_SCREENS.reduce(function (acc, screen) {
        var itemsAsFutureGridRows = splitDashboardLayoutItemsAsRenderedGridRows(acc, screen);
        return flatten(itemsAsFutureGridRows.map(function (futureGridRow) {
...
function unifyDashboardLayoutItemHeightsForScreen(items, screen) {
    ...
    var maxHeight = Math.max.apply(Math, __spreadArrays([0], heights));
    if (maxHeight === 0) {
        return items;
    }
...
var updateDashboardLayoutItemHeight = function (item, screen, maxHeight) {
    ...
    if (screen === "xs" && heightAsRatio > DASHBOARD_LAYOUT_MAX_HEIGHT_AS_RATIO_XS) {
        updatedColumn = __assign(__assign({}, updatedColumn), { size: __assign(__assign({}, updatedColumn.size), (_b = {}, _b[screen] = __assign(__assign({}, updatedColumn.size[screen]), { heightAsRatio: DASHBOARD_LAYOUT_MAX_HEIGHT_AS_RATIO_XS }), _b)) });
    }
...
export var getDashboardLayoutItemHeightForRatioAndScreen = function (size, screen) {
    var gridWidth = size.gridWidth, heightAsRatio = size.heightAsRatio;
...
export function getDashboardLayoutItemMaxGridWidth(item, screen) {
    ...
    for (var _i = 0, sectionItems_1 = sectionItems; _i < sectionItems_1.length; _i++) {
        var sectionItem = sectionItems_1[_i];
        ...
        if (newWidth <= DASHBOARD_LAYOUT_GRID_COLUMNS_COUNT) {
            ...
            gridRowWidth = (_a = sectionItem.sizeForScreen(screen)) === null || _a === void 0 ? void 0 : _a.gridWidth;
        }
    }
...
var getVisualizationSizeInfo = function (settings, insight) {
    ...
    if (isInsight(insight)) {
        sizeInfo = FullVisualizationCatalog.forInsight(insight).getSizeInfo(insight, fluidLayoutDescriptor, settings);
    }
    if (!sizeInfo) {

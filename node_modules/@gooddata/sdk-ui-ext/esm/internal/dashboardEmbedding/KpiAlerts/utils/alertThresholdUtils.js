...
export function thresholdFromDecimalToPercent(threshold) {
    ...
    var numberOfDecimalPlaces = getNumberOfDecimalPlaces(threshold);
    var numberOfWantedDecimalPlaces = Math.max(numberOfDecimalPlaces - 2, 0);
...
export function evaluateAlertTriggered(kpiMeasureResult, threshold, when) {
    var sanitizedValue = sanitizeValue(kpiMeasureResult);
    var sanitizedThreshold = sanitizeValue(threshold);
    switch (when) {
        case "aboveThreshold":
